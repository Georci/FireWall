...
library FixidityLib {
     ...
     * @param x 需要转换精度的int256类型固定点数
     * @param _originDigits 转换之前的精度(x中小数位数)
     * @param _destinationDigits 目标精度
     */
    function convertFixed(
        ...
        if (_originDigits > _destinationDigits) {
            ...
            //     10**38 < 2**128-1
            assert(
                x <= maxInt256() / int128(uint128(10) ** uint128(decimalDifference))
            );
            assert(
                x >= minInt256() / int128(uint128(10) ** uint128(decimalDifference))
            );
            return x * int128((uint128(10) ** uint128(decimalDifference)));
        }
